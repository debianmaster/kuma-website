        ...
        module Tabs
          class TabsBlock < ::Liquid::Block
            ...
            def render(context) # rubocop:disable Metrics/AbcSize, Metrics/MethodLength
              ...
              ::Jekyll::logger::info("#{file_path} #{@name}")
              environment['tabs'][file_path] ||= {}

              if environment['tabs'][file_path].key? @name
                #raise SyntaxError.new("There's already a {% tabs %} block with the name '#{@name}' on path #{file_path}.")
              end
              ...
              template_file = File.read(File.expand_path(site.config['jekyll-kuma-plugins']['tabs']['template'], site.source))
...
Liquid::Template.register_tag('tab', Jekyll::KumaPlugins::Liquid::Tags::Tabs::TabBlock)
Liquid::Template.register_tag('tabs', Jekyll::KumaPlugins::Liquid::Tags::Tabs::TabsBlock)
